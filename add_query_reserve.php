<?php
	...
	if(ISSET($_POST['add_guest'])){
		...
		$query2 = $conn->query("SELECT * FROM `transaction_data` WHERE `checkin` = '$checkin' && `reservation_id` = '$_REQUEST[reservation_id]' && `status` = 'Pending'") or die(mysqli_error());
		$row = $query2->num_rows;
		if($checkin < date("Y-m-d", strtotime('+8 HOURS'))){	
				...
				if($row > 0){
					echo "<div class = 'col-md-4'>
							...
							$q_date = $conn->query("SELECT * FROM `transaction_data` WHERE `status` = 'Pending'") or die(mysqli_error());
						...
						if($guest_id = $fetch['guest_id']){
							...
							$conn->query("INSERT INTO `transaction_data`(guest_id, reservation_id, status, checkin) VALUES('$guest_id', '$reservation_id', 'Pending', '$checkin')") or die(mysqli_error());
