                        ...
                        <div class="row align-items-center">
                            <div class="header-text mb-4">
                                <small>
                                    ...
                                    if (isset($_SESSION["admin-no-login-message"])) {
                                        echo $_SESSION["admin-no-login-message"];
                                        unset($_SESSION["admin-no-login-message"]);
...
if (isset($_POST["submit"]) and $_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    try {
        $query = "SELECT * FROM tbl_admin WHERE username = :username AND user_password=:user_password";
        ...
        if ($count == 1) {
            //user available
            $_SESSION["admin-login"] = "<div class='success'>Login Successfull</div>";
            $_SESSION["user"] = $username;//to check whether user is logged in and log out will unset it
            header("Location:" . HOMEURL . "admin/index.php");


        } else {
            //user not available
            $_SESSION["admin-login"] = "<div class='failed'>Username / Password Not Match</div>";
